...
export const RefactorModal: React.FC<RefactorModalProps> = ({
  isOpen,
  onClose,
  repoUrl,
  targetFile,
  ...
  const [refactorMarkdown, setRefactorMarkdown] = useState<string | null>(null);
  const pdfContentRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (isOpen && targetFile && !refactorMarkdown && !loading && !error) {
      generateRefactor();
    }
  }, [isOpen, targetFile]);
  ...
  const generateRefactor = async () => {
    ...
    setLoading(true);
    setError(null);
    setRefactorMarkdown(null);
    try {
      const res = await axios.post('/api/refactor-file', {
        repo_url: repoUrl,
        github_token: githubToken || undefined,
        gemini_token: geminiToken || undefined,
        target_file: targetFile
      });
      setRefactorMarkdown(res.data.refactorMarkdown);
    } catch (e: any) {
      console.error(e);
      setError(e.response?.data?.error || e.message || 'Failed to generate refactoring suggestions.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="p-4 border-b border-[#141414] flex items-center justify-between bg-white shrink-0 no-print">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-lg uppercase tracking-tight text-[#141414] flex items-center gap-2">
                  Code Optimizer <span className="px-1.5 py-0.5 bg-yellow-300 text-[10px] font-mono border border-[#141414]">AI</span>
                </h2>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={loading || !refactorMarkdown}
          ...
          <div className="flex-1 overflow-y-auto bg-[#F9F9F8] p-6 relative print:overflow-visible print:h-auto print:block print:bg-transparent print:p-0">
            {loading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center space-y-4 opacity-70">
                ...
                <div>
                  <p className="text-sm font-mono uppercase font-bold text-[#141414]">Analyzing Code Smells...</p>
                  <p className="text-xs text-[#141414]/60 max-w-sm mt-1">
                    The AI is parsing {targetFile} to find optimizations, bad practices, and modern syntax replacements.
                  </p>
                </div>
              </div>
            ) : error ? (
              ...
              >
                ...
                <div className="border-b-2 border-[#141414] pb-4 mb-6 flex justify-between items-end">
                  <div>
                    <h1 className="text-2xl font-bold uppercase tracking-tighter">Code Refactoring Report</h1>
                ...
                <div className="prose prose-sm prose-pre:bg-[#141414] prose-pre:text-[#F9F9F8] prose-pre:border prose-pre:border-[#141414] prose-a:text-[#2563EB] max-w-none text-[#141414] prose-pre:overflow-x-auto prose-pre:whitespace-pre-wrap break-words">
                  <ReactMarkdown>{refactorMarkdown}</ReactMarkdown>
                </div>
              </div>
            ) : null}
          </div>
